...
bool TaskGroupManager::migrate_memory_tracker_to_group(
    ...
    _task_groups[dst_group_id]->add_mem_tracker_limiter(mem_tracker);
    return true;
}

void TaskGroupManager::reset_query_ctx_scan_scheduler(QueryContext* query_ctx,
                                                      uint64_t dst_group_id) {
    std::shared_lock<std::shared_mutex> read_lock(_task_scheduler_lock);
    if (_tg_scan_sche_map.find(dst_group_id) != _tg_scan_sche_map.end()) {
        query_ctx->set_scan_task_scheduler(&(_tg_scan_sche_map[dst_group_id]));
    }
}
