...
PipelineTask* PriorityTaskQueue::try_take_unprotected(bool is_steal) {
    ...
    for (int i = 0; i < SUB_QUEUE_LEVEL; ++i) {
        ...
        if (!_sub_queues[i].empty()) {
            if (idx == -1 || cur_queue_vruntime < min_vruntime) {
                idx = i;
    ...
    // ss << "take task from prio task, level=" << idx << ", min runtime=" << min_vruntime 
    //     << ", min=" << _queue_level_min_vruntime << std::endl;
    // std::cout << ss.str();
...
Status PriorityTaskQueue::push(PipelineTask* task) {
    if (_closed) {
        return Status::InternalError("PriorityTaskQueue closed");
    }
    ...
    if (_sub_queues[level].empty() &&
        ...
        // std::stringstream ss;
        // ss << "ajust time when queue is empty, vruntime=" << _queue_level_min_vruntime << std::endl;
        // std::cout << ss.str();
        _sub_queues[level].adjust_runtime(_queue_level_min_vruntime);
    }
    // std::stringstream ss;
    // ss << "set level for task,level=" << level << ", " << "queue level vruntime=" << _sub_queues[level].get_vruntime() << std::endl;;
    // std::cout << ss.str();
