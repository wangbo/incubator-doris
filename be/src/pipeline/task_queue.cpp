...
WorkTaskQueue::WorkTaskQueue() : _closed(false) {
    ...
    _task_vruntime_level_limit[0] = BASE_TIME_SLICE;
    for (int i = 1; i < SUB_QUEUE_LEVEL; i++) {
        _task_vruntime_level_limit[i] = _task_vruntime_level_limit[i - 1] + BASE_TIME_SLICE * i;
    }
}
...
PipelineTask* WorkTaskQueue::try_take_unprotected(bool is_steal) {
    ...
    for (int i = 0; i < SUB_QUEUE_LEVEL; ++i) {
        if (_sub_queues[i].empty() && normal_vruntime_array[i] < min_schedule_time) {
            _sub_queues[i].adjust_queue_vruntime_to_min(min_schedule_time);
        }
    }
