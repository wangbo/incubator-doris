...
public:
    ...
    // vruntime(means virtual runtime) = runtime / _factor_for_normal
...
private:
    ...
    double _level_factor = 1; // depends on LEVEL_QUEUE_TIME_FACTOR
...
public:
    ...
    void update_statistics(PipelineTask* task, int64_t time_spent) override {
        // std::stringstream ss;
        // ss << "inc task and queue vruntime=" << time_spent << std::endl;
        // std::cout << ss.str();
