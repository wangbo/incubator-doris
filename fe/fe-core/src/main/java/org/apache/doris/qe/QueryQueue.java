...
public class QueryQueue {
    ...
    private Thread resetPropertyThread;
    ...
    public void start() {
        QueryQueue queryQueue = this;
        resetPropertyThread = new Thread(new Runnable() {
            public void run() {
                while (true) {
                    try {
                        queryQueue.resetQueueProperty(
                            VariableMgr.getDefaultSessionVariable().maxConcurrency,
                            VariableMgr.getDefaultSessionVariable().maxQueueSize,
                            VariableMgr.getDefaultSessionVariable().queueTimeout);
                        LOG.info(queryQueue.debugString());
                        Thread.sleep(30000);
                    } catch (Exception e) {
